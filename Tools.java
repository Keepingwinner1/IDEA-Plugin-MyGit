...
public class Tools {
    //将文件全部转移
    public static void readPaths(Path directoryPath,Path des) throws IOException {
    ...
    public static boolean isUnderDir(Path file){
        VersionControl instance = VersionControl.getInstance();
        Path src = instance.getProPath().resolve("src");
        File dir=src.toFile();
        try {
    ...
    public static void writeHashMapToFile(HashMap<Path, FileStatus> map, Path filePath) {
        for(HashMap.Entry<Path, FileStatus> entry : map.entrySet()){
            ...
            try {
                Files.writeString(filePath,entry.getKey()+"$"+status.getStatus()+"@"+status.getTimestamp()+"#"+status.getFilenum()+"\n",StandardOpenOption.APPEND);
            } catch (IOException e) {
    ...
    public static HashMap<Path, FileStatus> readHashMapFromFile(Path filePath) {
        HashMap<Path, FileStatus> map = new HashMap<Path, FileStatus>();
        try (BufferedReader br = new BufferedReader(new FileReader(filePath.toString()))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                String filenum = line.substring(line.indexOf('#') + 1).trim();
                map.put(Paths.get(path),new FileStatus(status,timestamp,filenum));
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
